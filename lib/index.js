...
export async function convert({ sourceFile, destinationFile, gameDirectory, itemDefinitions, textureMode, objectName }) {
  ...
  if (defaultMaterial && document.getRoot().listMeshes().length == 1) {
    // Let's create a map between Materials and Meshes
    const meshesPerMaterial = {};
    const nodesPerMaterial = {};
    const defaultMaterialMesh = document.getRoot().listMeshes()[0]
    .setName(defaultMaterial.getName());
    const defaultMaterialMeshNode = document.getRoot().listNodes()
      .find(node => node.getMesh() == defaultMaterialMesh)
      .setName(defaultMaterialMesh.getName());
    meshesPerMaterial[defaultMaterial.getName()] = defaultMaterialMesh;
    nodesPerMaterial[defaultMaterial.getName()] = defaultMaterialMeshNode;

    // Let's assign one Mesh per Material
    defaultMaterialMesh.listPrimitives().forEach(
      (primitive, idx) => {
        const primitiveMaterial = primitive.getMaterial();
        const primitiveMaterialName = primitiveMaterial.getName();
        if (primitiveMaterial.getName() != defaultMaterialMesh.getName()) {
          ...
          if (!meshesPerMaterial[primitiveMaterialName]) {
            ...
            const meshNode = document.createNode(primitiveMaterialName)
              .setMesh(mesh)
              .setTranslation(defaultMaterialMeshNode.getTranslation())
              .setRotation(defaultMaterialMeshNode.getRotation());
          ...
          defaultMaterialMesh.removePrimitive(primitive);
        } else {
          ...
          console.log('Default mesh indices, may have elements:', primitive.getIndices());
    ...
    for (const node of Object.values(nodesPerMaterial)) {
